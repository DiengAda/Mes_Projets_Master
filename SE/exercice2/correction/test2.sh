...
# Crée une grande arborescence
# $1 = racine
# $2 = profondeur
# $3 = nb objets à chaque niveau
# $4 = taux de répertoires (ex : 4 pour 1 répertoire tous les 4 fichiers)
creer_grande_arbo ()
{
    [ $# != 4 ] && fail "ERREUR SYNTAXE creer_grande_arbo"
    local racine="$1" prof="$2" nbobj="$3" taux="$4"
    local i
	...
	do
	    if [ $(( (i + prof ) % taux )) = 0 ]
	    then creer_grande_arbo $racine/dir$i $prof $nbobj $taux
	    else creer_fichier $racine/fich$i $((2999 * (i % taux)))
	    fi
	done
    fi
}
...
# Ce test est omis car il peut échouer pour des facteurs exogènes
# (ex : profondeur de /usr/include aboutit à des chemins trop grands)
...
annoncer_test 3.3 "grande arborescence"
nettoyer
creer_grande_arbo $TMP.d 7 5 2
$PROG $TMP.d > $TMP.out 2> $TMP.err || fail "erreur détectée"
est_vide $TMP.err	|| fail "il ne doit rien y avoir sur stderr"
reproduire_et_comparer $TMP.out $TMP.d
echo OK
